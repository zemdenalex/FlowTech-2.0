...
const Schools = () => {
  ...
  return (
    <section id="schools" className="py-20 md:py-36 bg-bg-primary">
        ...
        {SCHOOL_SECTIONS.map((section, idx) => (
          ...
          >
            <div className="px-4 md:px-8 lg:px-12 max-w-7xl mx-auto">
              {/* Title */}
              <h2 className="text-xl sm:text-2xl md:text-4xl lg:text-5xl font-medium tracking-[0.05em] md:tracking-[0.1em] uppercase text-white mb-3 md:mb-4">
                {t(section.titleKey)}
              </h2>
              ...
              {/* Features list */}
              <ul className="space-y-3 md:space-y-4 max-w-3xl border-l-2 border-[#C8A55C]/30 pl-4">
                {Array.from({ length: section.featureCount }, (_, i) => (
                  <li key={i} className="flex items-start gap-3 md:gap-4">
                    {/* Gradient bullet */}
                    <span className="mt-1.5 md:mt-2 flex-shrink-0 w-1.5 h-1.5 md:w-2 md:h-2 rounded-full bg-[#C8A55C]" />
              ...
              {section.extraSections?.map((extra, eIdx) => (
                <div key={eIdx} className="mt-8 md:mt-10">
                  <h3 className="text-lg md:text-2xl font-medium uppercase text-white/90 mb-3 md:mb-4 tracking-wider">
                    {t(extra.labelKey)}
                  </h3>
                  <ul className="space-y-3 max-w-3xl border-l-2 border-[#C8A55C]/30 pl-4">
